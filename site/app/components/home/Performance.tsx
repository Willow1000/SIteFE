...
export default function Performance() {
  ...
  return (
    <section id="performance" ref={sectionRef} className="bg-[#1a1d29] py-24 md:py-32 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto">
        <div className={`flex flex-col lg:flex-row lg:items-center lg:justify-between mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="max-w-2xl mb-8 lg:mb-0">
            <h2 className="text-[#4a9d7e] text-sm font-bold tracking-[0.2em] uppercase mb-4">Real Results</h2>
            <h3 className="text-3xl md:text-5xl font-bold text-white mb-6">Unrivaled Market Performance</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          {stats.map((stat, index) => (
            <div 
              key={index} 
              className={`bg-[#252836]/40 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-gray-800 hover:border-gray-700 transition-all duration-1000 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
              }`}
              style={{ transitionDelay: `${index * 150}ms` }}
            >
              <p className="text-gray-500 text-sm font-medium mb-3 tracking-wide">{stat.label}</p>
              <div className="flex items-end space-x-2">
                <span className={`text-2xl md:text-4xl font-bold ${stat.color}`}>{stat.value}</span>
                {stat.trend === 'up' && (
        ...
        {/* Chart Visualization */}
        <div className={`bg-[#252836]/60 backdrop-blur-md rounded-[2.5rem] p-8 md:p-12 border border-gray-800 relative group transition-all duration-1000 delay-500 ${
          isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
        }`}>
          <div className="absolute top-8 right-12 flex items-center space-x-6 text-xs font-bold uppercase tracking-widest text-gray-500">
          ...
          <div className="mt-8 flex justify-between text-gray-500 text-sm font-bold uppercase tracking-widest">
            <span>Oct 2024</span>
            <span>Nov 2024</span>
            <span>Dec 2024</span>
            <span>Jan 2025</span>
