...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" ref={sectionRef} className="bg-[#1a1d29] py-24 md:py-32 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto">
        <div className={`text-center mb-20 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <h2 className="text-[#4a9d7e] text-sm font-bold tracking-[0.2em] uppercase mb-4">Social Proof</h2>
          <h3 className="text-3xl md:text-5xl font-bold text-white mb-6">Trusted by 50k+ Traders</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div
              key={testimonial.id}
              className={`bg-[#252836]/40 backdrop-blur-md rounded-3xl p-10 border border-gray-800 relative transition-all duration-1000 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'
              }`}
              style={{ transitionDelay: `${index * 200}ms` }}
            >
              {/* Quote Icon */}
              <div className="absolute top-8 right-10 text-[#4a9d7e]/10">
              ...
              <p className="text-gray-300 text-lg italic leading-relaxed mb-10 relative z-10">
                "{testimonial.text}"
              </p>

              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 bg-[#4a9d7e] rounded-full flex items-center justify-center text-white font-bold text-lg shadow-[0_0_15px_rgba(74,157,126,0.3)]">
                  {testimonial.avatar}
                </div>
                <div>
                  <h5 className="text-white font-bold">{testimonial.name}</h5>
                  <p className="text-[#4a9d7e] text-xs font-bold uppercase tracking-widest">{testimonial.role}</p>
                </div>
              </div>
            </div>
          ))}
